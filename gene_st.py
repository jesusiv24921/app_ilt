...
import csv
import tempfile
# Función para detectar automáticamente el delimitador del archivo CSV
...
if file is not None:
        ...
        with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
            ...
            nombre_archivo = tmp_file.name

        # Detectar automáticamente el delimitador
        delimitador = detectar_delimitador(nombre_archivo)

        # Leer el archivo CSV usando el delimitador detectado
        df = pd.read_csv(nombre_archivo, sep=delimitador, header=None)
        df_=df.iloc[0:2,0:2]
